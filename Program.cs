...
// Parte una nota en dos
        ...
        Data_OpenUtau[j]="Length="+length_note/2;
        //Busca el final de esta nota
        int k=j+1;
        while(k<Data_OpenUtau.Length && !Data_OpenUtau[k].StartsWith("[#0")){
            k++;
        }
        //Guarda en un array aux la data
        string[] Data_aux_1=new string[k];//Creo un array que va a tener los primeros k elementos
        ...
        Data_note[k+1]="Length="+(length_note/2  +  length_note%2).ToString();
        //Guarda los cambios
        Data_OpenUtau =new string[Data_aux_1.Length+Data_aux_2.Length+Data_note.Length];
        Array.Copy(Data_OpenUtau,0,Data_aux_1,0,k);
        Array.Copy(Data_OpenUtau,k+1,Data_note,0,k-j);
        Array.Copy(Data_OpenUtau,k+k-j,Data_aux_2,0,k-j);
//
        for (int i=0; i < Data_OpenUtau.Length; i++){
